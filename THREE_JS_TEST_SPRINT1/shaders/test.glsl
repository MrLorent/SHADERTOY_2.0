...
//On pourrait même envoyer tout ça en une fois un uBoxInfos, uSphereInfos (?)
uniform mat4 uBoxInvMatrix[N_BOXES];
uniform mat4 uSphereInvMatrix[N_SPHERES];
uniform vec3 uBoxColor[N_BOXES];
uniform vec3 uSphereColor[N_SPHERES];
uniform int uBoxType[N_BOXES];
uniform int uSphereType[N_SPHERES];
...
//---------------------------------------------------------------------------------------
{
    ...
    for(int i =0; i<N_SPHERES; i++){
        vec3 vecPosition = (mat4(uSphereInvMatrix[i])* vec4(spheres[i].position.xyz, 1)).xyz;
		d = SphereIntersect( spheres[i].radius, vecPosition, rayOrigin, rayDirection );
        ...
        {	
            ...
            // transfom normal back into world space
            normal = normalize(normal);
            hitNormal = normalize(transpose(inverse(mat3(uSphereInvMatrix[i]))) * normal);
...
//-----------------------------------------------------------------------
{
    ...
    for(int i=0; i<N_BOXES; i++){
        boxes[i]  = Box( vec3(-86.0, -85.0, -80.0), vec3(0, 85.0, 80.0), z,uBoxColor[i], uBoxType[i]);// Tall Mirror Box Left
    }

    for(int i=0; i<N_SPHERES; i++){
        spheres[i] = Sphere(50.0, vec3(200, 100, -200), z, uSphereColor[i], 0.0, uSphereType[i]);// White Ball
    }
